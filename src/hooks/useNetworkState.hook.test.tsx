...
function UseNetworkStateExampleMock({ controller }: Props) {
  ...
  return (
    <>
      ...
      <button onClick={actions.start} data-testid={BUTTON_START_DATA_TEST_ID}>start</button>
      <button onClick={actions.end} data-testid={BUTTON_END_DATA_TEST_ID}>end</button>
      <button onClick={actions.abort} data-testid={BUTTON_ABORT_DATA_TEST_ID}>abort</button>
      <button onClick={actions.resetError} data-testid={BUTTON_RESET_ERROR_DATA_TEST_ID}>
        reset error
      </button>
      <button
        onClick={() => { actions.setError(ERROR_MOCK) }}
        data-testid={BUTTON_SET_ERROR_DATA_TEST_ID}
      >
        set error
      </button>
      <button onClick={actions.start} data-testid={BUTTON_SET_LOADING_DATA_TEST_ID}>set loading</button>
      <button onClick={() => { actions.setData(DATA_MOCK) }} data-testid={BUTTON_SET_DATA_DATA_TEST_ID}>
        set data
      </button>
      ...
      <button
        onClick={() => { if(controller) actions.setController(controller) }}
...
const fetchMock = jest.fn(() => Promise.resolve(DATA_MOCK));
